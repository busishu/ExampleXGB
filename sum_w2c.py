...
if __name__ == '__main__':
    ...
    fr = open('valid/valid_nolabel.txt', 'r')
    user2 = []
    line = fr.readline()
    while line:
    ...
    fru = open('train/lb_and_count_weibo.txt', 'r')
    uuu = dict()
    line = fru.readline()
    p = 0
    while line:
    ...
    print uuu
    print len(uuu.keys())
    fru = open('valid/valid_status.txt', 'r')
    line = fru.readline()
    while line:
        uc = line.split(",")[0]
        if uc  in user2 :
    ...
    print "load OK"
    fr = open('train/train_labels.txt','r')
    user = []
    line = fr.readline()
    while line:
    ...
    fr = open('valid/valid_status.txt', 'r')
    line = fr.readline()
    i = 0
    while line:
    ...
    fr.close()
    fr = open('valid/w2v_sentence.txt','w')
    for k in range(3200):
        s = str(user[k])
        for wq in m[k]:
